...
public class Main {
    ...
    /**
     * Helper method to add initial books and users to the library system.
     *
     * @param librarySystem The library system to add data to.
     */
    private static void addInitialData(LibrarySystem librarySystem) {
        try {
            ...
            librarySystem.addBookWithTitleAndAuthorList("The Lord of the Rings",
                    List.of(new Author("J.R.R. Tolkien")));
            librarySystem.addBookWithTitleAndAuthorList("The Silmarillion", List.of(new Author("J.R.R. Tolkien")));
            librarySystem.addBookWithTitleAndAuthorList("The Children of Húrin",
                    List.of(new Author("J.R.R. Tolkien")));
            librarySystem.addBookWithTitleAndAuthorList("The Fall of Gondolin",
                    List.of(new Author("J.R.R. Tolkien")));

            librarySystem.addStudentUser("Frodo Baggins", true);
            librarySystem.addStudentUser("Samwise Gamgee", true);
            librarySystem.addStudentUser("Meriadoc Brandybuck", true);
            librarySystem.addStudentUser("Peregrin Took", false);

            librarySystem.addFacultyMemberUser("Gandalf", "Wizard");
            librarySystem.addFacultyMemberUser("Aragorn", "King");
    ...
    private static User logInUser(Scanner scanner, LibrarySystem librarySystem) {
        ...
        if (isFacultyMember) {
            ...
            librarySystem.addFacultyMemberUser(userName, department);
            System.out.println("Faculty member added successfully!");
        } else {
            librarySystem.addStudentUser(userName, false);
            System.out.println("Student added successfully!");
        }
        try {
            userLoggedIn = librarySystem.findUserByName(userName);
    ...
    private static void addBook(Scanner scanner, LibrarySystem librarySystem) {
        ...
        try {
            if (authorList.size() == 1) {
                librarySystem.addBookWithTitleAndNameOfSingleAuthor(title, authors[0].trim());
            } else {
                librarySystem.addBookWithTitleAndAuthorList(title, authorList);
            }
     ...
     * @param librarySystem The library system.
     * @param userLoggedIn The user who is logged in.
     * @param isFacultyMember Indicates whether the logged-in user is a faculty member.
     */
    private static void extendLending(Scanner scanner, LibrarySystem librarySystem, User userLoggedIn, boolean isFacultyMember) {
        if (isFacultyMember) {
